    ...
    func getMission(query: MissionRequestModel, completion: @escaping (Result<MissionModelAll, Error>) -> Void) {
     ...
     private   func mapperMissionModelResponseToMissionModel(response:MissionModelResponse)->MissionModelAll?{
            ...
            let missionModel =   docs.map { item in
                ...
                return MissionModel(id:item.id ,flightNumber: flightNumber, totalPages:response.totalPages, name: item.name, details: item.details, dateUTC: dateString, smallPatch: item.links?.patch?.small, largePatch: item.links?.patch?.large,wikipedia:item.links?.wikipedia, success: item.success)
            }
